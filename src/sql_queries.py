...
full_names_list = [(f'nazvanie{i}.{random.choice(extensions)}', random.randint(0, 1))
...
instr(full_names.name, \'.\') + 1 ---> 10+1
substr(full_names.name, instr(full_names.name, \'.\') + 1)) ---> .mp3
length(.mp3) ---> 4
substr(full_names.name, length(13-4)) ---> nazvanie1
...
cursor.execute('UPDATE full_names \
               ...
               (SELECT status FROM short_names \
                WHERE substr(full_names.name, 1, \
                        length(full_names.name)-length(substr(full_names.name, instr(full_names.name, \'.\') + 1))) \
                        = short_names.name)')
db_connection.commit()
end_time = time.time()
